...
LegControllerCPG::LegControllerCPG(mjModel * m, mjData * d, char * s1, char * a1, char * s2, char * a2, CPGNode * cpg) 
	: ModelController(m, d), ctrlR(2, 0, 0), ctrlT(0.25, 0, 0.05) {
...
void LegControllerCPG::step() {
	double angle1 = data->sensordata[sensor1] + 90;
	double angle2 = data->sensordata[sensor2] + mjPI / 4;
	double angleAvg = (angle1 - angle2) / 2;
	double s = data->sensordata[sensorTouch];

	double measuredR = 0.1 * sin(angleAvg) + 0.2 * sin(acos(cos(angleAvg) / 2));
	...
	double setT = 0;// x;
	double setR = 0.15;// y > 0 ? 0.275 : 0.125;

	double dr = ctrlR.calculateOutput(data->time, setR, measuredR);
	double dt = 0;// ctrlT.calculateOutput(data->time, setT, totalT);

	data->ctrl[motor1] = dt - dr;
	data->ctrl[motor2] = dt + dr;

	printf("%f -> %f | %f -> %f\n", measuredR, setR, totalT, setT);
	//printf("%f\n", measuredR);
}